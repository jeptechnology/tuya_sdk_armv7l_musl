...
// 事件传递数据原型，tlv结构，信息暂由事件发送、接收双方自行协商
typedef struct {
    int type;           // 发送数据类型
...
typedef int (*event_subscribe_cb)(event_raw_data_t *data);
 ...
 * @return int: 0成功，非0，请参照tuya error code描述文档 
 */
int publish_event(const char* name, event_raw_data_t *data);
 ...
 * @return int: 0成功，非0，请参照tuya error code描述文档 
 */
int subscribe_event(const char *name, const char *desc, const event_subscribe_cb cb, int is_emergency);
 ...
 * @return int: 0成功，非0，请参照tuya error code描述文档 
 */
int unsubscribe_event(const char *name, const char *desc, event_subscribe_cb cb);
